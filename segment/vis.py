...
if __name__ == '__main__':
    ...
    img0 = cv2.imread('C:/Users\lzy06\Desktop\zxq/relaticdata/00000.png')
    ...
    sess = rt.InferenceSession('./model/best11.onnx')
    input_name = sess.get_inputs()[0].name
    label_name = sess.get_outputs()[0].name
    pred = sess.run([label_name], {input_name: data.astype(np.float32)})[0]
    pred = np.squeeze(pred) #(7, 8400)
    pred = np.transpose(pred, (1, 0)) # (8400, 7)
    pred_class = pred[..., 4:] #(8400, 3)
    print('pred3', pred_class.shape)
    pred_conf = np.max(pred_class, axis=-1) #(8400,)
    print('pred3', pred_conf.shape)
    pred = np.insert(pred, 4, pred_conf, axis=-1) #(8400, 8)
